...
+function updateCleanAndPutInOrder() {
  ...
  +function cloneFreshAndRemoveUnneeded() {
	  if (process.argv.length > 2 &&
			  process.argv[2].startsWith("-l")) {
		  // This is to fetch a local version of ace (if the -local option was supplied)
		  buildroot = path.join(__dirname, 'ace-temp');
		  rm('-rf', buildroot);
		  exec('(mkdir ' + buildroot + ' && cd ' + buildroot + ')');
		  exec('cp -r ' + path.join(__dirname, '../../ace/build/src-*noconflict/') + ' ' + buildroot);
	  }
	  else {
		  // Or, clone ace from the official github repo
		  rm('-rf', buildroot)
