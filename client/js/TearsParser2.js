...
function resolveAliases(expectedType, node, ret_undef = false) {
  ...
  res = {
    culprit: evalChain.substring(2), // If somethings goes wrong this is the culprit chain.
    value: res
  }
...
function addConstDefAlias(deftype, type, identifier, value) {
  ...
  if (getConstDefAlias(identifier) == undefined || deftype == 'alias') {   // an alias can be redefined to mimic function calls.
    ...
    constDefAlias.unshift({
      deftype: deftype, // keep old aliases for debug purposes.
      type: type,
...
function resample_signal_unitTest() {
  ...
  P.push({
    ...
    P: [1, 10, 20, 55, 61, 100, 131, 261],
    exp: [0, 0, 5, 5, -3, 9, 15,]
  }); // Last point should have been cut off.
...
function partition_events_by_intervals(P, S) {
  // TODO: There should be a global switch whether we consume 
  // the events that are matched. Investigations are required to
  // see if it should be done here or in the GA evaluation.
  //
  // Returns:
  ...
  for (var is = 0; is < len; is++) {      // One partition for each interval in S
    var ip = 0; // TODO: this can(should) be optimized
    var s = S[is];
    var part = [];
    var begin = s[0];
    var end = s[1];
...
function evaluate_relative_to_P(eventsExpression_E, timebase_P) {
  ...
  moduleContext.currentGA = {
    G: timebase_P,    // We are not interested in using
    tw: Number.MAX_VALUE
  };          // within (means end of sequence).
...
function evaluate_transition(E0str, Pv0, T0, E1, t1) {
  ...
  */
  ...
  var plots = [{
    exprStr: "Events = Lhs Events -> Rhs Events within t:(" +
  ...
  if (traceSeq) {
    console.log("TearsParser2.js::evaluate_transition OUT: ",
      JSON.stringify({
        failTraces: fails, passes: passes,
  ...
  semantics2.addOperation('eval', {
    ...
    */
    Intervals_conj: function (lhs_intervals, lhs_timeFilter, conj, rhs_intervals, rhs_timeFilter) {
      function createPlot(SLE, SRE, conj, res) {
        ...
        var plot = {
          exprStr: "Intervals = Intervals " + conj +
    ...
    */
    IntervalsExpr_logop: function (signal1, op, signal2) {
      ...
      */
      ...
      plots.push({
        exprStr: "Intervals = Signal  " + op.sourceString +
    ...
    */
    IntervalFunction_between: function (_key, _lp, start, _comma, end, _rp) {
      /*
      TODO: Remember WHY we disencuraged use like this, its needed for the REG patterns
        if(moduleContext.currentGA != undefined){
      ...
      */
      ...
      var plots = [{
        exprStr: "Intervals = between(Events|Offset, Events|Offset):" +
    ...
    */
    Signal_sigOpSig: function (lhs, op, rhs) {
      ...
      var plots = [];
      plots.push({
        exprStr: "Signals = Signal " + op.sourceString + " Signal: (" +
...
*/
    ...
    */
    SignalFunction_derivative: function (_keyw, _l, param, _c, signal, _r) {
      ...
      var plots = [{
        exprStr: "Signal = derivative(Timout?, Signal): derivative(" +
    ...
    SignalFunction_abs: function (_keyw, _l, signal, _r) {
      ...
      var S = {
        xAxis: SE.value.xAxis,   // Do not modify the original signal...
        values: SE.value.values
      };//(we will exchange values pointer)
      var absValues = [];
      S.values.forEach(v => {
    ...
    */
    SignalFunction_counts: function (_kw, _lp, events, _comma, intervals, _rp) {
      ...
      var plots = [{
        exprStr: "Signal = Count(Events, Intervals): Count(" +
    ...
    SignalFunction_maxVal(_maxVal, _lp, signals, _rp) {
      ...
      // Create plots
      var plots = [{
        exprStr: "Signal = maxVal(<list of signals>): maxVal(" + signals.sourceString + ")",
        signals: [
        ]
      }];
    ...
    */
    SignalFunction_select: function (_fname, _lp, control, _comm1, signal1, _comm2, signal2, _rp) {
      ...
      var plots = [{
        exprStr: "Signal = select(signal_control, signal_true, signal_false): " + sigLabel,
        signals: [
          {
    ...
    GuardedAssertion_eventGA: function (_when, guard, _shall, _verify, assertion) {
      ...
      var retVal = {
        times: {
          ...
          fail: [],
          ignored: []
        },  // New! Extra assertion events not fail and not pass.
        guards: [],
        assertions: [],
        signals: moduleContext.usedSignals
      };
      ...
      for (var rep = 0; rep < assertion.children.length; rep++) {
        ...
        if (case_nr < 4) {
          ...
          */
          ...
          options["_Skipped E:"] = { type: 'r', y: 1, markers: 'i' };
          retVal.guards.unshift({
            exprStr: "Evaluation : when " + guard.sourceString + " " + assertion.sourceString + "",
            events: [
              { name: "Guard", events: G },
    ...
    */
    Events_andIntervals: function (events, _and, intervals) {
      ...
      var plots = [{
        exprStr: "Events = Events and Intervals: (" +
    ...
    */
    Events_conj: function (events1, conj, events2) {
      ...
      var plots = [{
        exprStr: "Events = Events " + op + " Events:(" +
    ...
    Sequence: function (_seq, events1, withins1, stores1, _arrow, events2, withins2, stores2) {
      ...
      if (moduleContext.currentGA != undefined) {
        ...
        if (store_id != undefined) {
          stored_timepoints.push({
            node: 0, id: store_id, expr: E1, events: res.passes,
        ...
        if (store_id != undefined) {
          stored_timepoints.push({
            node: 0, id: store_id, expr: E1, events: Pv0,
            valid: undefined, seqnrs: undefined
          });
        }
      }//is guard
    ...
    */
    EventsExpr_intervalAnd: function (intervals, _and, events) {
      ...
      var plots = [{
        exprStr: "Events = Intervals and Events: (" +
    ...
    */
    EventsExpr_intervalFor: function (intervals, forexp) {
      ...
      var plots = [{
        exprStr: "Events = Intervals for: (" + intervals.sourceString +
          ") " + forexp.sourceString,
        events: [
    ...
    */
    EventsExpr_def: function (definedEvents) {
      ...
      res.plots = [{
        exprStr: "Events = Def: " + definedEvents.eval(),
        events: [
    ...
    */
    EventFunctions_risingEdge: function (_re, _lp, intervals, _rp) {
      ...
      options["_Result"] = { type: 'R', y: 1, markers: "S" };
      var plots = [{
        exprStr: "Events = rising_edge(Intervals|Signal): rising_edge("
