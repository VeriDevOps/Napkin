...
export function derivate_backward(data) {
  // backward approximating derivate with threshold to smooth out
    ...
    if(data.hasOwnProperty("xAxis") && data.hasOwnProperty("values")) {
        ...
        var timestamps = [];
        var values = [];
        timestamps.push(0); // zero dy/dt at start
        values.push(0);
        if(data.values.length < 2){    // Signals must be larger than 2 samples.
          console.log("operators::derivate::WARNING signal with < 2 sample points to derivate function...")
          return [[0],[0]];
        }
        var xt_1 = data.xAxis[0];
        var yt_1 = data.values[0];
        for (let i = 1; i < data.xAxis.length; i++) {
          var xt = data.xAxis[i];
          var yt = data.values[i];
          var deltax = xt - xt_1;
          if (deltax < xThreshold) {
               continue; // skip samples in between
          }
          timestamps.push(xt);
          values.push((yt - yt_1)/(xt - xt_1));
          xt_1 = xt;
          yt_1 = yt
        }//endfor
...
export function apply_bit_mask(mask, signal) {
    ...
    for (var i = 0; i < signal.values.length; i++) {
        var masked = signal.values[i] & mask;
        if (masked != oldValue) {
            values.push(masked);
            timestamps.push(signal.xAxis[i]);
            oldValue = masked;
        }// only store new values
    }//for
    // If the last point has the same value as previous ones,
    // we need to add it last in resulting signal.
    var lastSignalIndex = signal.values.length - 1;
    if (timestamps[timestamps.length - 1] < signal.xAxis[lastSignalIndex]){
        var masked = signal.values[lastSignalIndex] & mask;
        values.push(masked);
        timestamps.push(signal.xAxis[lastSignalIndex]);
    }  // add last point
...
export function between(startTimestamps, endTimestamps) {
  ...
  for (let i=0; i<startTimestamps.length; i++) {
    ...
    if (nextEndIndex === endTimestamps.length) {
      intervals.push([startTime, EvalContext.getInstance().getXmax()]);
      break;
    }
    else {
