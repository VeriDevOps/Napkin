<template>
  <div id="gaeditor" style="height: 192px; resize: inherit">
    <b-container fluid>
      <b-row>
        <b-col>
          <b-alert
            :variant="evauationStatus"
            data-toggle="tooltip"
            :title="globalDefinitionsFile"
            data-placement="auto right"
            on.show.bs.tooltip="function(){alert('The tooltip is about to be shown.');"
            show
            >{{ lastError }}</b-alert
          >
        </b-col>
...
-->
          <b-form-file
            :state="Boolean(exampleFile)"
            accept=".js"
            label="kalle"
            @change="loadExample"
            aria-label="Small"
            :plain="false"
            placeholder="Drop file or click to browse to load example"
            v-model="exampleFile"
            ref="fileinput"
          >
            Load</b-form-file
  ...
  -->
      <li>
        <a href="#" @click.prevent="onClick('editUsedSignals')">
          Edit used signals
        </a>
      </li>

      <li>
        <a v-show="false" href="#" @click.prevent="onClick('loadGA')"
          >Upload and edit T-EARS a file</a
        >
      </li>
      <hr />
      <li>
        <a :v-show="gafile_url" href="#" @click.prevent="onClick('reloadGA')"
          >Revert to T-EARS file on server</a
        >
      </li>
      <li>
        <a
          :v-show="gafile_url"
          href="#"
          @click.prevent="onClick('saveEditedGA')"
          >Save T-EARS file to server file</a
        >
      </li>
      <hr />
      <li v-if="editorID == 'tears-editor-horizontal'">
        <a href="#" @click.prevent="onClick('save')">
          Save Editor Contents and Relevant Signals
        </a>
      </li>
      <li>
        <a
          v-if="editorID == 'tears-editor-horizontal'"
          v-show="false"
          href="#"
          @click.prevent="onClick('loadExample')"
        >
          Load Contents and Relevant Signals
        </a>
      </li>
      <hr v-if="editorID == 'tears-editor-horizontal'" />
      <li>
        <a href="#" @click.prevent="onClick('enableDefaults')"
          >Enable Loaded Global Definitions</a
        >
      </li>

      <li>
        <a href="#" @click.prevent="onClick('disableDefaults')"
          >Disable Global Definitions
        </a>
      </li>
      <hr />
      <li>
        <a href="#" @click.prevent="onClick('editDefaults')">
          Global Definitions -> Editor Contents
        </a>
      </li>
      <li>
        <a href="#" @click.prevent="onClick('replaceDefaults')">
          Editor Contents -> Global Definitions
        </a>
      </li>
      <hr />
      <li>
        <a href="#" @click.prevent="onClick('evaluate')">
          Evaluate Expression
        </a>
      </li>
    </vue-context>
...
export default {
  ...
  methods: {
    ...
    onClick(choice) {
      ...
      switch (choice) {
        ...
        case "editDefaults":
          if (
            confirm(
              "Overwrite editor context with global defs (and turn globals off)."
            )
          ) {
            defBlock.active = false;
        ...
        case "replaceDefaults":
          if (
            confirm(
              "Overwrite current global definitions with editor contents."
            )
          ) {
            defBlock.active = true;
    ...
    reloadGA() {
      ...
      var r = confirm(
        "Do you really want to replace current editor content with \n" + gafile
      );
      if (r == false) return;
    ...
    saveEditedGA() {
      ...
      if (gafile == null) return;
      var r = confirm(
        "Do you really want to replace content on server for \n" + gafile
      );
      if (r == false) return;
      let editorText = this.tearsEditor.getValue();
      bus.listeners["bt-save-resp"] = self.remote_save_req_response;
      bus.socket.send(
        JSON.stringify({ type: "bt-save-ga", gatext: editorText, path: gafile })
      );
    ...
    createUnitTest() {
      ...
      for (let s of usedSignals) {
        ...
        var serialized_signal = {
          pretty_print: curr.pretty_print,
          newName: curr.name,
          xAxis: curr.xAxis,
          values: curr.values,
        ...
        doc += ",";
        if (curr.timestamps && !timestampv.includes(curr.timestamps)) {
      ...
      doc +=
        ',"globals": ' +
        JSON.stringify(
          EvalContext.getInstance().getDefaultDefinitionBlock().content
        );
    ...
    onEditExternalText(text, targetEditor, evaluate = false, askfirst = true) {
      let self = this;
      console.log(
        "GAEditor.vue::onEditExternalText ",
        targetEditor,
        self.editorID
      );
      if (targetEditor != self.editorID) return;
      if (askfirst == true) {
    ...
    loadExample(e) {
      ...
      reader.onload = function (e) {
        ...
        EvalContext.getInstance().forceUnitTestSetTimeStamps(
          signals["TimeStamps"]
        );
    ...
    loadGA(e) {
      //i.e load GA text from uploaded file
      var self = this;
      var file = e.target.files[0];
    ...
    setupAutocompletion(editor) {
      ...
      var defCompleter = {
        ...
        getCompletions: function (editor, session, pos, prefix, callback) {
          callback(
            null,
            EvalContext.getInstance().getConstDefAliasCompletions()
          );
        },
      };
    ...
    evaluate(useEditorText = true) {
      ...
      let editorText = "";
      if (useEditorText == true) editorText = self.tearsEditor.getValue();
      ...
      switch (res.status) {
        ...
        default:
          console.log(
            "GAEditor.js::evaluate() cannot handle evaluation " +
              JSON.stringify(res)
          );
      }
      //console.groupEnd();
    ...
    on_window_resize() {
      ...
      if (leftPane == null || rightPane == null || thisContainer == null) {
        debugger;
        return;
      }
      /*
      ...
      thisContainer.style.height =
        rightPane.offsetHeight - thisContainer.offsetTop - 40 + "px";
    ...
    on_focus_changed(newFocus) {
      ...
      self.lastError =
        "Press Ctrl+Enter to evaluate! (The plots from the last evaluation are shown until then.)";
    },
    on_start_loading(leftToLoad) {
      //   console.log("GAEditor.vue::on_start_loading:: Event received");
      this.evauationStatus = "warning";
      this.lastError =
        "Please wait.... Loading and Processing Signal File.... " +
        leftToLoad +
  ...
  computed: {
    ...
    globalDefinitionsFile: function () {
      ...
      let defBlock = EvalContext.getInstance().getDefaultDefinitionBlock();
      if (defBlock.active) return "Loaded Definitions:" + defBlock.fileName;
