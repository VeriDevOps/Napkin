...
def root_path():
    ...
    print("Trying to return ", static_file_root + "/index.html")
    filename = 'index.html'
...
def evaluate():
    ...
    try:
        ...
        logfile = os.getenv('NAPKIN_BASE') + os.getenv('DEFAULT_LOG_DIR') + \
            request.query.logfile.replace('"', '')
...
TEMPLATES.clear()
print(
    f"Starting Napkin rest server at {rest_srv_host}:{rest_srv_port}", flush=True)
